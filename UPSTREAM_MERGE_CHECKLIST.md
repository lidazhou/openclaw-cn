...
|------|--------|---------|---------|------|------|----------|------|
| ⬜ | P0 | #8590 | v2026.X.X | Gateway/Security: redact sensitive session/path details from `status` responses for non-admin clients; full details remain available to `operator.admin`. | SECURITY | LOW | 安全修复，必须合并 |
| ⬜ | P0 | #17682 | v2026.X.X | Gateway/Control UI: preserve requested operator scopes for Control UI bypass modes (`allowInsecureAuth` / `dangerouslyDisableDeviceAuth`) when device identity is unavailable, preventing false `missing scope` failures on authenticated LAN/HTTP operator sessions. | SECURITY | MEDIUM | 安全/认证修复，必须合并 |
| ⬜ | P0 | #17587 | v2026.X.X | LINE/Security: fail closed on webhook startup when channel token or channel secret is missing, and treat LINE accounts as configured only when both are present. | SECURITY | LOW | 安全修复，必须合并 |
| ⬜ | P0 | #16042 | v2026.X.X | Config/Gateway: make sensitive-key whitelist suffix matching case-insensitive while preserving `passwordFile` path exemptions, preventing accidental redaction of non-secret config values like `maxTokens` and IRC password-file paths. | SECURITY | MEDIUM | 配置/安全修复，必须合并 |
| ⬜ | P0 | #15707 | v2026.X.X | Gateway/Agent: reject malformed `agent:`-prefixed session keys (for example, `agent:main`) in `agent` and `agent.identity.get` instead of silently resolving them to the default agent, preventing accidental cross-session routing. | SECURITY | MEDIUM | 核心路由安全修复，必须合并 |
| ⬜ | P0 | #8593 | v2026.X.X | Gateway/Chat: harden `chat.send` inbound message handling by rejecting null bytes, stripping unsafe control characters, and normalizing Unicode to NFC before dispatch. | SECURITY | LOW | 安全/Unicode修复，必须合并 |
| ⬜ | P0 | #15703 | v2026.X.X | Gateway/Send: return an actionable error when `send` targets internal-only `webchat`, guiding callers to use `chat.send` or a deliverable channel. | CRITICAL-BUG | LOW | 核心网关错误处理修复，必须合并 |
| ⬜ | P0 | #17404 | v2026.X.X | Agents/Context: apply configured model `contextWindow` overrides after provider discovery so `lookupContextTokens()` honors operator config values (including discovery-failure paths). | CORE-FEATURE | MEDIUM | 核心Agent上下文配置修复，必须合并 |
| ⬜ | P0 | #17586 | v2026.X.X | Agents/Context: derive `lookupContextTokens()` from auth-available model metadata and keep the smallest discovered context window for duplicate model ids, preventing cross-provider cache collisions from overestimating session context limits. | CORE-FEATURE | MEDIUM | 核心Agent上下文处理修复，必须合并 |
| ⬜ | P0 | #16803 | v2026.X.X | Agents/OpenAI: force `store=true` for direct OpenAI Responses/Codex runs to preserve multi-turn server-side conversation state, while leaving proxy/non-OpenAI endpoints unchanged. | CORE-FEATURE | LOW | OpenAI/Codex模型状态修复，必须合并 |
| ⬜ | P0 | #17672 | v2026.X.X | Memory/FTS: make `buildFtsQuery` Unicode-aware so non-ASCII queries (including CJK) produce keyword tokens instead of falling back to vector-only search. | CORE-FEATURE | LOW | CJK/Unicode相关修复，必须合并 |
| ⬜ | P0 | #17603 | v2026.X.X | Auto-reply/Compaction: resolve `memory/YYYY-MM-DD.md` placeholders with timezone-aware runtime dates and append a `Current time:` line to memory-flush turns, preventing wrong-year memory filenames without making the system prompt time-variant. | CORE-FEATURE | LOW | 记忆/压缩系统修复，必须合并 |
| ⬜ | P0 | #17633 | v2026.X.X | Auto-reply/Compaction: resolve `memory/YYYY-MM-DD.md` placeholders with timezone-aware runtime dates and append a `Current time:` line to memory-flush turns, preventing wrong-year memory filenames without making the system prompt time-variant. | CORE-FEATURE | LOW | 记忆/压缩系统修复，必须合并 |
| ⬜ | P0 | #16659 | v2026.X.X | Agents: return an explicit timeout error reply when an embedded run times out before producing any payloads, preventing silent dropped turns during slow cache-refresh transitions. | CRITICAL-BUG | LOW | 核心Agent超时处理修复，必须合并 |
| ⬜ | P0 | #14447 | v2026.X.X | Group chats: always inject group chat context (name, participants, reply guidance) into the system prompt on every turn, not just the first. Prevents the model from losing awareness of which group it's in and incorrectly using the message tool to send to the same group. | CRITICAL-BUG | LOW | 群聊上下文修复，必须合并 |
| ⬜ | P0 | #17673 | v2026.X.X | Browser/Agents: when browser control service is unavailable, return explicit non-retry guidance (instead of "try again") so models do not loop on repeated browser tool calls until timeout. | CRITICAL-BUG | LOW | 工具调用循环修复，必须合并 |
| ⬜ | P0 | #17150 | v2026.X.X | Subagents: use child-run-based deterministic announce idempotency keys across direct and queued delivery paths (with legacy queued-item fallback) to prevent duplicate announce retries without collapsing distinct same-millisecond announces. | CORE-FEATURE | MEDIUM | 子Agent重复通知修复，必须合并 |
| ⬜ | P0 | #10942 | v2026.X.X | Telegram: omit `message_thread_id` for DM sends/draft previews and keep forum-topic handling (`id=1` general omitted, non-general kept), preventing DM failures with `400 Bad Request: message thread not found`. | CHANNEL-FIX | LOW | Telegram渠道关键修复，必须合并 |
| ⬜ | P0 | #16789 | v2026.X.X | Telegram: replace inbound `<media:audio>` placeholder with successful preflight voice transcript in message body context, preventing placeholder-only prompt bodies for mention-gated voice messages. | CHANNEL-FIX | LOW | Telegram语音消息修复，必须合并 |
| ⬜ | P0 | #16154 | v2026.X.X | Telegram: retry inbound media `getFile` calls (3 attempts with backoff) and gracefully fall back to placeholder-only processing when retries fail, preventing dropped voice/media messages on transient Telegram network errors. | CHANNEL-FIX | LOW | Telegram媒体下载重试修复，必须合并 |
| ⬜ | P0 | #17218 | v2026.X.X | Telegram: finalize streaming preview replies in place instead of sending a second final message, preventing duplicate Telegram assistant outputs at stream completion. | CHANNEL-FIX | LOW | Telegram流式输出修复，必须合并 |
| ⬜ | P0 | #17679 | v2026.X.X | Telegram: disable block streaming when `channels.telegram.streamMode` is `off`, preventing newline/content-block replies from splitting into multiple messages. | CHANNEL-FIX | LOW | Telegram流式输出配置修复，必须合并 |
| ⬜ | P0 | #17622 | v2026.X.X | Discord: preserve channel session continuity when runtime payloads omit `message.channelId` by falling back to event/raw `channel_id` values for routing/session keys, so same-channel messages keep history across turns/restarts. Also align diagnostics so active Discord runs no longer appear as `sessionKey=unknown`. | CHANNEL-FIX | LOW | Discord会话连续性修复，必须合并 |
| ⬜ | P0 | #17365 | v2026.X.X | Discord: dedupe native skill commands by skill name in multi-agent setups to prevent duplicated slash commands with `_2` suffixes. | CHANNEL-FIX | LOW | Discord技能命令去重修复，必须合并 |
| ⬜ | P0 | #17491 | v2026.X.X | Web UI/Agents: hide `BOOTSTRAP.md` in the Agents Files list after onboarding is completed, avoiding confusing missing-file warnings for completed workspaces. | CHANNEL-FIX | LOW | Web UI修复，必须合并 |
| ⬜ | P0 | #17194 | v2026.X.X | Memory/QMD: scope managed collection names per agent and precreate glob-backed collection directories before registration, preventing cross-agent collection clobbering and startup ENOENT failures in fresh workspaces. | CORE-FEATURE | HIGH | QMD记忆系统修复，需对比本地实现 |
| ⬜ | P0 | #7010 | v2026.X.X | Auto-reply/WhatsApp/TUI/Web: when a final assistant message is `NO_REPLY` and a messaging tool send succeeded, mirror the delivered messaging-tool text into session-visible assistant output so TUI/Web no longer show `NO_REPLY` placeholders. | CHANNEL-FIX | MEDIUM | WhatsApp/TUI/Web UI消息显示修复，必须合并 |
| ⬜ | P0 | #15664 | v2026.X.X | Cron: infer `payload.kind="agentTurn"` for model-only `cron.update` payload patches, so partial agent-turn updates do not fail validation when `kind` is omitted. | CORE-FEATURE | LOW | Cron任务修复，必须合并 |
| ⬜ | P0 | #4519 | v2026.X.X | TUI: make searchable-select filtering and highlight rendering ANSI-aware so queries ignore hidden escape codes and no longer corrupt ANSI styling sequences during match highlighting. | CHANNEL-FIX | LOW | TUI ANSI处理修复，必须合并 |
| ⬜ | P0 | #4986 | v2026.X.X | TUI/Windows: coalesce rapid single-line submit bursts in Git Bash into one multiline message as a fallback when bracketed paste is unavailable, preventing pasted multiline text from being split into multiple sends. | CHANNEL-FIX | LOW | TUI/Windows粘贴修复，必须合并 |
| ⬜ | P0 | #5782 | v2026.X.X | TUI: suppress false `(no output)` placeholders for non-local empty final events during concurrent runs, preventing external-channel replies from showing empty assistant bubbles while a local run is still streaming. | CHANNEL-FIX | LOW | TUI并发运行显示修复，必须合并 |
| ⬜ | P0 | #17515 | v2026.X.X | TUI: preserve copy-sensitive long tokens (URLs/paths/file-like identifiers) during wrapping and overflow sanitization so wrapped output no longer inserts spaces that corrupt copy/paste values. | CHANNEL-FIX | LOW | TUI复制粘贴修复，必须合并 |
| ⬜ | P0 | #17466 | v2026.X.X | TUI: preserve copy-sensitive long tokens (URLs/paths/file-like identifiers) during wrapping and overflow sanitization so wrapped output no longer inserts spaces that corrupt copy/paste values. | CHANNEL-FIX | LOW | TUI复制粘贴修复，必须合并 |
| ⬜ | P0 | #17505 | v2026.X.X | TUI: preserve copy-sensitive long tokens (URLs/paths/file-like identifiers) during wrapping and overflow sanitization so wrapped output no longer inserts spaces that corrupt copy/paste values. | CHANNEL-FIX | LOW | TUI复制粘贴修复，必须合并 |
| ⬜ | P0 | #7180 | v2026.X.X | Onboarding: add Moonshot (.cn) auth choice and keep the China base URL when preserving defaults. | MODEL | LOW | 中国相关Provider支持，必须合并 |
| ⬜ | P0 | #8994 | v2026.X.X | Docs: mirror the landing page revamp for zh-CN (features, quickstart, docs directory, network model, credits). | NICE-TO-HAVE | LOW | 中文文档更新，可选合并 |
| ⬜ | P0 | #8702 | v2026.X.X | Heartbeat: allow explicit accountId routing for multi-account channels. | CORE-FEATURE | LOW | 核心心跳多账户路由修复，必须合并 |
| ⬜ | P0 | #8432 | v2026.X.X | TUI/Gateway: handle non-streaming finals, refresh history for non-local chat runs, and avoid event gap warnings for targeted tool streams. | CHANNEL-FIX | MEDIUM | TUI/Gateway事件处理修复，必须合并 |
| ⬜ | P0 | #8193 | v2026.X.X | Telegram: honor session model overrides in inline model selection. | CHANNEL-FIX | LOW | Telegram模型选择修复，必须合并 |
| ⬜ | P0 | #7178 | v2026.X.X | Web UI: resolve header logo path when `gateway.controlUi.basePath` is set. | CHANNEL-FIX | LOW | Web UI路径修复，必须合并 |
| ⬜ | P0 | #8484 | v2026.X.X | Onboarding: infer auth choice from non-interactive API key flags. | CORE-FEATURE | LOW | 配置系统修复，必须合并 |
| ⬜ | P0 | #8540 | v2026.X.X | Cron: deliver announce runs directly, honor delivery mode, and respect wakeMode for summaries. | CORE-FEATURE | MEDIUM | Cron任务交付修复，必须合并 |
| ⬜ | P0 | #8392 | v2026.X.X | Telegram: include forward_from_chat metadata in forwarded messages and harden cron delivery target checks. | CHANNEL-FIX | LOW | Telegram转发消息修复，必须合并 |
| ⬜ | P0 | #7313 | v2026.X.X | Feishu: add Feishu/Lark plugin support + docs. | CHANNEL-FIX | LOW | 飞书渠道支持，必须合并 |
| ⬜ | P0 | #3160 | v2026.X.X | Memory: implement the opt-in QMD backend for workspace memory. | CORE-FEATURE | HIGH | QMD记忆系统实现，需对比本地实现 |
| ⬜ | P0 | #7372 | v2026.X.X | Config: allow setting a default subagent thinking level via `agents.defaults.subagents.thinking` (and per-agent `agents.list[].subagents.thinking`). | CORE-FEATURE | LOW | 配置系统增强，必须合并 |
| ⬜ | P0 | #8202 | v2026.X.X | Docs: zh-CN translations seed + polish, pipeline guidance, nav/landing updates, and typo fixes. | NICE-TO-HAVE | LOW | 中文文档更新，可选合并 |
| ⬜ | P0 | #6995 | v2026.X.X | Docs: zh-CN translations seed + polish, pipeline guidance, nav/landing updates, and typo fixes. | NICE-TO-HAVE | LOW | 中文文档更新，可选合并 |
| ⬜ | P0 | #6619 | v2026.X.X | Docs: zh-CN translations seed + polish, pipeline guidance, nav/landing updates, and typo fixes. | NICE-TO-HAVE | LOW | 中文文档更新，可选合并 |
| ⬜ | P0 | #7242 | v2026.X.X | Docs: zh-CN translations seed + polish, pipeline guidance, nav/landing updates, and typo fixes. | NICE-TO-HAVE | LOW | 中文文档更新，可选合并 |
| ⬜ | P0 | #7303 | v2026.X.X | Docs: zh-CN translations seed + polish, pipeline guidance, nav/landing updates, and typo fixes. | NICE-TO-HAVE | LOW | 中文文档更新，可选合并 |
| ⬜ | P0 | #7415 | v2026.X.X | Docs: zh-CN translations seed + polish, pipeline guidance, nav/landing updates, and typo fixes. | NICE-TO-HAVE | LOW | 中文文档更新，可选合并 |
| ⬜ | P0 | #1 | v2026.X.X | Security: require operator.approvals for gateway /approve commands. | SECURITY | LOW | 安全修复，必须合并 |
| ⬜ | P0 | #7226 | v2026.X.X | fix(webchat): respect user scroll position during streaming and refresh | CHANNEL-FIX | LOW | Web UI滚动修复，必须合并 |
| ⬜ | P0 | #7466 | v2026.X.X | Telegram: recover from grammY long-poll timed out errors. | CHANNEL-FIX | LOW | Telegram长轮询超时修复，必须合并 |
| ⬜ | P0 | #7473 | v2026.X.X | Agents: repair malformed tool calls and session transcripts. | CORE-FEATURE | MEDIUM | 核心Agent工具调用修复，必须合并 |
| ⬜ | P0 | #7277 | v2026.X.X | fix(agents): validate AbortSignal instances before calling AbortSignal.any() | CORE-FEATURE | LOW | 核心Agent信号处理修复，必须合并 |
| ⬜ | P0 | #7475 | v2026.X.X | Media understanding: skip binary media from file text extraction. | CORE-FEATURE | LOW | 媒体处理修复，必须合并 |
| 🔲 | P2 | #16724 | v2026.X.X | Plugins: expose `llm_input` and `llm_output` hook payloads so extensions can observe prompt/input context and model output usage details. | NICE-TO-HAVE | MEDIUM | 插件系统增强，可选合并 |
| 🔲 | P2 | #14447 | v2026.X.X | Subagents: nested sub-agents (sub-sub-agents) with configurable depth. Set `agents.defaults.subagents.maxSpawnDepth: 2` to allow sub-agents to spawn their own children. Includes `maxChildrenPerAgent` limit (default 5), depth-aware tool policy, and proper announce chain routing. | NICE-TO-HAVE | HIGH | 子Agent嵌套功能增强，可选合并 |
| 🔲 | P2 | #17092 | v2026.X.X | Slack/Discord/Telegram: add per-channel ack reaction overrides (account/channel-level) to support platform-specific emoji formats. | NICE-TO-HAVE | LOW | 渠道表情符号格式支持，可选合并 |
| 🔲 | P2 | #14535 | v2026.X.X | Cron/Gateway: add finished-run webhook delivery toggle (`notify`) and dedicated webhook auth token support (`cron.webhookToken`) for outbound cron webhook posts. | NICE-TO-HAVE | MEDIUM | Cron Webhook功能增强，可选合并 |
| 🔲 | P2 | #16986 | v2026.X.X | Channels: deduplicate probe/token resolution base types across core + extensions while preserving per-channel error typing. | NICE-TO-HAVE | HIGH | 代码重构，可选合并 |
